...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        model_input = latents * 0.476986 # hunyuan like ??
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # main_print(f"{hunyuan_teacher_disable_cfg} teacher_kwargs-- hidden_states: {teacher_kwargs['hidden_states'].shape}, encoder_hidden_states: {teacher_kwargs['encoder_hidden_states'].shape}, timestep: {teacher_kwargs['timestep'].shape}, encoder_attention_mask: {teacher_kwargs['encoder_attention_mask'].shape}")
            x = [noisy_model_input[i] for i in range(noisy_model_input.size(0))]
            use_cond = random.random() > cfg_drop_p
            if use_cond:
            ...
            # print(f"uncond_model_pred: {uncond_model_pred.shape}")
        ...
        with torch.no_grad():
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                # teacher_output = cond_teacher_output + guidance_cfg * (cond_teacher_output - uncond_teacher_output)
...
def main(args):
    ...
    main_print(f"no_split_modules:{no_split_modules}, fsdp_kwargs:{fsdp_kwargs}")
