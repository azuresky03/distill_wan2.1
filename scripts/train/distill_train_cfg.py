...
from scripts.train.latent_datasets import (LatentDataset, latent_collate_function)
...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        # latents = all_gather(latents, dim=2).contiguous()
        # print(f'rank {rank}: Converged latents {latents.shape}, latents: {latents[0][0][0][0][:10]}, {latents.sum()}')
        ...
        print(f'rank-{dist.get_rank()}: noisy_model_input: {noisy_model_input.sum()}')
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            main_print(f"using guidance_cfg: {guidance_cfg}")
            if guidance_cfg:
        ...
        huber_c = 0.001
        # loss = loss.mean()
        loss = (torch.mean(torch.sqrt((model_pred.float() - teacher_output.float())**2 + huber_c**2) - huber_c) /
                gradient_accumulation_steps)
    ...
    model_pred, _ = solver.euler_style_multiphase_pred(noisy_model_input, model_pred, index, multiphase)

    print(f"before save: \nrank-{dist.get_rank()}: model input: {noisy_model_input[0][0][0][0][:10]};\n modelpredori: {model_pred_org[0][0][0][:10]}\n modelpred: {model_pred[0][0][0][:10]}")
    dist.barrier()
    ...
    if debug_save_dir:
        ...
        if debug_save_dir:
            ...
            with torch.no_grad():
                for value,name in [(latents,"latents"),(model_pred,"model_pred"),(teacher_output,"teacher_output"),(noisy_model_input,"noisy_model_input"),(model_pred_org,"model_pred_org")]:
                    save_video_path = f"{debug_save_dir}/{name}——rank{dist.get_rank()}.mp4"
                    # dist.barrier()
                    # if len(value.shape) == 4:
                    #     value = value.unsqueeze(0)
                    # print(f"\nrank-{dist.get_rank()}: name: {name}; value: {value[0][0][0][0][:10]}, {value[0].sum()} \nsaved at : {save_video_path}")
                    # if len(value.shape) == 4:
                    ...
                    video = videos[0]
                    # print(f"value shape: {value.shape}, video shape: {video[0].shape}")
                    cache_video(tensor=video[None],save_file=save_video_path,fps=16,nrow=1,normalize=True,value_range=(-1, 1))
        dist.barrier()
...
def main(args):
    ...
    if not args.resume_from_weight:
        weights = transformer_org.state_dict()
    else:
        weights = load_weights(args.resume_from_weight)
    main_print(f"--> loaded Wan model from {args.ckpt_dir}")
    config = dict(transformer_org.config)
    config["guidance_embed"] = True
    transformer = WanModelCFG(**config)
    results = transformer.load_state_dict(weights,strict=False)
    main_print(f"Missing keys: {results.missing_keys}")
    main_print(f"Unexpected keys: {results.unexpected_keys}")
    transformer.requires_grad_(True)
    del transformer_org, weights
    ...
    fsdp_kwargs, no_split_modules = get_dit_fsdp_kwargs(
        ...
        (WanAttentionBlock_CFG,),
    )

    teacher_fsdp_kwargs, teacher_no_split_modules = get_dit_fsdp_kwargs(
        teacher_transformer,
        args.fsdp_sharding_startegy,
        args.use_lora,
        args.use_cpu_offload,
    ...
    if args.gradient_checkpointing:
        ...
        apply_fsdp_checkpointing(teacher_transformer, teacher_no_split_modules, args.selective_checkpointing)
        if args.use_ema:
    ...
    for step in range(init_steps + 1, args.max_train_steps + 1):
        ...
        if rank == 0:
            a = random.randint(low_cfg,up_cfg)
        ...
        if DEBUG:
            ...
            debug_save_dir = f"/vepfs-zulution/yinhaoyu/Wan_zpp/debug/exp14/{step}/"
