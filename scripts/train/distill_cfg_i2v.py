...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        # print(f"latents: {get_str(latents)}, encoder_hidden_states: {get_str(encoder_hidden_states)}, attention_mask: {get_str(attention_mask)}, encoder_attention_mask: {get_str(encoder_attention_mask)}, y: {get_str(y)}, clip_feature: {get_str(clip_feature)}")
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # print(f"{model_input.device} computed model_pred success!")

        with torch.no_grad():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                cond_teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask,y=y,clip_fea=clip_feature)[0].float()
                # print(f"CON{model_input.device} computed cond_teacher_output success!")
                # print(f"null_encoded: {null_encoded.shape}, encoder_hidden_states: {encoder_hidden_states.shape}, encoder_attention_mask: {encoder_attention_mask.shape}")
                uncond_teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask,y=y,clip_fea=clip_feature)[0].float()
                # print(f"UN{model_input.device} computed uncond_teacher_output success!")
        ...
        # print(f"one loop {model_input.device}")
        dist.barrier()
...
def main(args):
    ...
    null_encoded = null_encoded.to(device).unsqueeze(0)
    ...
    from scripts.train.model.model_seq import WanAttentionBlock as WanAttentionBlock2
