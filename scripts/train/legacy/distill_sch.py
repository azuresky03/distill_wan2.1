...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        index = torch.randint(0, num_euler_timesteps-1, (bsz, ), device=model_input.device).long()
        if sp_size > 1:
            broadcast(index)
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # print(f"device{noisy_model_input.device} noisy_model_input: {noisy_model_input.shape}, encoder_hidden_states: {teacher_kwargs['encoder_hidden_states'].shape}, timestep: {teacher_kwargs['timestep'].shape}, encoder_attention_mask: {teacher_kwargs['encoder_attention_mask'].shape}")
            x = [noisy_model_input[i] for i in range(noisy_model_input.size(0))]
            use_cond = not cfg_drop
            if CFG_TYPE==0:
                if use_cond:
                    model_pred = transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0] #43200
                else:
                    ### cfg
                    model_pred = transformer(x,timesteps,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0]
            else:
                con_model_pred = transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0]
                uncond_model_pred = transformer(x,timesteps,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0]
                model_pred = uncond_model_pred + guidance_cfg * (con_model_pred - uncond_model_pred)
        ...
        with torch.no_grad():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                if CFG_TYPE==0:
                    if use_cond:
                        teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0].float()
                    else:
                        teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0].float()
                else:
                    cond_teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0].float()
                    uncond_teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0].float()
                    teacher_output = uncond_teacher_output + guidance_cfg * (cond_teacher_output - uncond_teacher_output)
            ...
            noise_scheduler._step_index = int(index.item())
            noise_scheduler.last_sample = None
            x_prev = noise_scheduler.step(sample=noisy_model_input, model_output=teacher_output, timestep=int(index.item()),return_dict=False)[0]
        ...
        with torch.no_grad():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                if ema_transformer is not None:
                    ...
                    # target_pred = transformer(x_prev_list, timesteps_prev, None, max_seq_len, batch_context=encoder_hidden_states, context_mask=encoder_attention_mask)[0]
                    if CFG_TYPE==0:
                        # main_print("Using drop, CFG_TYPE==0")
                        if use_cond:
                            target_pred = transformer(x_prev_list,timesteps_prev,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0] #43200
                        else:
                            ### cfg
                            main_print("using null_encoded")
                            target_pred = transformer(x_prev_list,timesteps_prev,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0]
                    else:
                        con_model_pred_tar = transformer(x_prev_list,timesteps_prev,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0]
                        uncond_model_pred_tar = transformer(x_prev_list,timesteps_prev,None,max_seq_len,batch_context=null_encoded,context_mask=encoder_attention_mask)[0]
                        target_pred = uncond_model_pred_tar + guidance_cfg * (con_model_pred_tar - uncond_model_pred_tar)
            # noise_scheduler._step_index = int(index.item())-1
            # noise_scheduler.last_sample = noisy_model_input
    ...
    if debug_save_dir:
        # infer_model_output = model_pred_org.clone()
        # with torch.no_grad():
        #     with torch.autocast("cuda", dtype=torch.bfloat16):
        #         for ijk in range(10):
        #             infer_model_output = transformer([infer_model_output],timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask)[0]

        torch.cuda.empty_cache()
        if debug_save_dir and int(os.environ["LOCAL_RANK"]) <= 0:
            assert vae is not None
            with torch.no_grad():
                for value,name in [(latents,"latents"),(model_pred,"model_pred"),(target,"target"),(target_pred,"target_pred"),(teacher_output,"teacher_output"),(noisy_model_input,"noisy_model_input"),(model_pred_org,"model_pred_org")]:
                    video = vae.decode([value.squeeze(0).float()])[0]
                    print(f"{name} value shape: {value.shape}, video shape: {video[0].shape}")
                    cache_video(tensor=video[None],save_file=f"{debug_save_dir}/{name}.mp4",fps=16,nrow=1,normalize=True,value_range=(-1, 1))
        dist.barrier()
...
def main(args):
    ...
    null_encoded = torch.load(args.null_encoded_path, map_location="cpu")
    ...
    noise_scheduler.num_inference_steps = -1e6
    ...
    transformer = load_wan(
        ...
        device_id=device,
        rank=rank,
        weight_path=args.resume_from_weight,
    )
    transformer.requires_grad_(True)

    teacher_transformer = load_wan(
        config=cfg,
        checkpoint_dir=args.ckpt_dir,
        device_id=device,
        rank=rank,
    ...
    # no_split_modules = (WanAttentionBlock,)
    ...
    for step in range(init_steps + 1, args.max_train_steps + 1):
        ...
        if DEBUG:
            ...
            debug_save_dir = f"/vepfs-zulution/zhangpengpeng/cv/video_generation/Wan2.1/debug/exp2/{step}"
