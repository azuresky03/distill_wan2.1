...
class WanExtractor(WanModelCFG):
    ...
    def forward(
        ...
        return [u.float() for u in x] if not cls else [ [u.float() for u in x], cls_xs ]
...
class WanGanModel(nn.Module):
    def __init__(self, wan_base, num_layers=10, layer_idxs=[3,6,9], dtype=torch.bfloat16, device=None, 
        ...
        self.self_attn1 = SelfAttention(dim=config["dim"]).to(device).to(dtype)
        self.ffw = nn.Sequential(
            nn.LayerNorm(config["dim"]),
            nn.Linear(config["dim"], config["dim"] * 4),
            nn.GELU(),
            nn.Linear(config["dim"] * 4, config["dim"])).to(device).to(dtype)
        self.self_attn2 = SelfAttention(dim=config["dim"]).to(device).to(dtype)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(dim//16, 1)).to(device).to(dtype)

    def forward(self, x, t, context, seq_len, clip_fea=None, y=None):
        extracted_features = self.extractor(x, t, context, seq_len, clip_fea=clip_fea, y=y, cls=True, cls_layers=self.cls_layers)[1]
        ...
        # 使用自注意力模块进行进一步处理
        compressed_features = self.self_attn1(compressed_features.unsqueeze(0)).squeeze(0) + compressed_features
        compressed_features = self.ffw(compressed_features) + compressed_features
        compressed_features = self.self_attn2(compressed_features.unsqueeze(0)).squeeze(0) + compressed_features
        compressed_features = compressed_features.mean(0)  # shape: [1, dim]
...
def test_gan_model():
    ...
    # 加载基座模型
    print("加载基座模型...")
    model_base = WanModelCFG.from_pretrained("/vepfs-zulution/models/Wan2.1-T2V-14B")
    ...
    with amp.autocast("cuda",dtype=torch.bfloat16):
        outputs = gan_model(x, t, context, seq_len)
